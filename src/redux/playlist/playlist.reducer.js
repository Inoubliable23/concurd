...
const initialState = {
	playlists: {
		'1': {
			...
			videosIdsOrder: ['7wNPJJoIzP4', '2', '3', '4']
		}
	}
}
	...
	return produce(state, draft => {
		switch (type) {
			...
			case ADD_VIDEO:
				...
				if (playlistToUpdate) {
					playlistToUpdate.videos[payload.video.id] = payload.video;
				}
				break;
			case REMOVE_VIDEO:
				...
				if (playlistToDelete) {
					const index = playlistToDelete.videosIdsOrder.indexOf(videoId);
					if (index > -1) {
						playlistToDelete.videosIdsOrder.splice(index, 1);
					}
